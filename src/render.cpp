...
void Render::DrawPath(io2d::output_surface &surface) const {
  ...
  const io2d::brush foreBrush {io2d::rgba_color::orange};
...
void Render::DrawEndPosition(io2d::output_surface &surface) const {
  if (m_Model.path.empty())
    return;
  io2d::render_props aliased {io2d::antialias::none};
  const io2d::brush foreBrush {io2d::rgba_color::red};

  auto pb = io2d::path_builder {};
  pb.matrix(m_Matrix);

  pb.new_figure(
    {static_cast<float>(m_Model.path.back().x), static_cast<float>(m_Model.path.back().y)});
  ...
  surface.stroke(foreBrush,
                 io2d::interpreted_path {pb},
                 std::nullopt,
                 std::nullopt,
                 std::nullopt,
                 aliased);
...
void Render::DrawStartPosition(io2d::output_surface &surface) const {
  ...
  pb.new_figure(
    {static_cast<float>(m_Model.path.front().x), static_cast<float>(m_Model.path.front().y)});
  float constexpr l_marker = 0.01f;
  pb.rel_line({l_marker, 0.f});
  pb.rel_line({0.f, l_marker});
  pb.rel_line({-l_marker, 0.f});
...
void Render::DrawHighways(io2d::output_surface &surface) const {
  ...
  for (auto [roadWay, roadType] : m_Model.Roads())
    if (auto rep_it = m_RoadReps.find(roadType); rep_it != m_RoadReps.end()) {
      const auto &[repBrush, repDashes, repMetricWidth] = rep_it->second;
      auto &way = ways[roadWay];
      const auto width = repMetricWidth > 0.f ? (repMetricWidth * m_PixelsInMeter) : 1.f;
      auto sp = io2d::stroke_props {width, io2d::line_cap::round};
      surface.stroke(repBrush, PathFromWay(way), std::nullopt, sp, repDashes);
    }
}
...
io2d::interpreted_path Render::PathFromMP(const Model::Multipolygon &mp) const {
  ...
  for (const auto way_num : mp.outer)
    commit(ways[way_num]);
  for (const auto way_num : mp.inner)
    commit(ways[way_num]);
...
void Render::BuildRoadReps() {
  ...
  const auto types = {R::Motorway,
                      ...
                      R::Footway};
  for (auto type : types) {
    auto &[repBrush, repDashes, repMetricWidth] = m_RoadReps[type];
    repBrush = io2d::brush {RoadColor(type)};
    repMetricWidth = RoadMetricWidth(type);
    repDashes = RoadDashes(type);
  }
}

void Render::BuildLanduseBrushes() {
  m_LanduseBrushes.insert_or_assign(Model::Landuse::Commercial,
                                    io2d::brush {io2d::rgba_color {233, 195, 196}});
  m_LanduseBrushes.insert_or_assign(Model::Landuse::Construction,
                                    io2d::brush {io2d::rgba_color {187, 188, 165}});
  m_LanduseBrushes.insert_or_assign(Model::Landuse::Grass,
                                    io2d::brush {io2d::rgba_color {197, 236, 148}});
  m_LanduseBrushes.insert_or_assign(Model::Landuse::Forest,
                                    io2d::brush {io2d::rgba_color {158, 201, 141}});
  m_LanduseBrushes.insert_or_assign(Model::Landuse::Industrial,
                                    io2d::brush {io2d::rgba_color {223, 197, 220}});
  m_LanduseBrushes.insert_or_assign(Model::Landuse::Railway,
                                    io2d::brush {io2d::rgba_color {223, 197, 220}});
  m_LanduseBrushes.insert_or_assign(Model::Landuse::Residential,
...
static io2d::point_2d ToPoint2D(const Model::Node &node) noexcept {
  return {static_cast<float>(node.x), static_cast<float>(node.y)};
}
