...
void Model::LoadData(const std::vector<std::byte> &xml) {
  ...
  if (const auto bounds = doc.select_nodes("/osm/bounds"); !bounds.empty()) {
    const auto node = bounds.first().node();
    m_MinLat = strtod(node.attribute("minlat").as_string(), nullptr);
    m_MaxLat = strtod(node.attribute("maxlat").as_string(), nullptr);
    m_MinLon = strtod(node.attribute("minlon").as_string(), nullptr);
    m_MaxLon = strtod(node.attribute("maxlon").as_string(), nullptr);
  } else
  ...
  for (const auto &node : doc.select_nodes("/osm/node")) {
    ...
    m_Nodes.back().y = strtod(node.node().attribute("lat").as_string(), nullptr);
    m_Nodes.back().x = strtod(node.node().attribute("lon").as_string(), nullptr);
  }
      ...
      if (const auto name = std::string_view {child.name()}; name == "nd") {
        ...
        if (category == "railway") {
          ...
          m_Buildings.back().outer = {way_num};
                   ...
                   (category == "natural" && (type == "wood" || type == "tree_row" ||
  ...
  for (const auto &relation : doc.select_nodes("/osm/relation")) {
    ...
    for (auto child : node.children()) {
      if (const auto name = std::string_view {child.name()}; name == "member") {
...
void Model::AdjustCoordinates() {
  ...
  for (auto &[nodeX, nodeY] : m_Nodes) {
    nodeX = (lon2xm(nodeX) - min_x) / m_MetricScale;
    nodeY = (lat2ym(nodeY) - min_y) / m_MetricScale;
  }
}
  ...
  if (nodes.empty()) {
    ...
    for (int i = 0; i < open_ways.size(); ++i)
      if (!used[i]) {
        ...
        if (const auto way_tail = way_nodes.back(); way_head == tail || way_tail == tail) {
          used[i] = true;
...
static std::vector<int> Track(std::vector<int> &open_ways, const Model::Way* ways) {
  ...
  std::vector used(open_ways.size(), false);
  std::vector<int> nodes;
  if (TrackRec(open_ways, ways, used, nodes))
    for (int i = 0; i < open_ways.size(); ++i)
      if (used[i])
...
void Model::BuildRings(Multipolygon &mp) {
  ...
  auto process = [&](std::vector<int> &ways_nums) {
    ...
    while (!open.empty()) {
      ...
      if (new_nodes.empty())
        break;
      open.erase(std::remove_if(open.begin(), open.end(), [](auto v) { return v < 0; }),
                 open.end());
